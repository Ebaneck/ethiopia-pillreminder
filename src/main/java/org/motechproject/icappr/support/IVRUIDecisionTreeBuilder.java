...
public class IVRUIDecisionTreeBuilder {
	...
	private void createDecisionTree() {
		...
		rootTransition.setDestinationNode(new Node().setNoticePrompts(
				new Prompt[] { new AudioPrompt().setAudioFileUrl(settings
						.getCmsliteUrlFor(SoundFiles.CONTINUE_PROMPTS)) })
				.setTransitions(
						new Object[][] { { "1", getContinueTransition() },
								{ "3", getStopTransition() } }));
	...
	private Transition getStopTransition() {
		...
		transition
				.setDestinationNode(new Node().setPrompts(new AudioPrompt()
						.setAudioFileUrl(settings
	...
	private Transition getContinueTransition() {
		...
		transition
				.setDestinationNode(new Node().setNoticePrompts(new Prompt[] { new AudioPrompt()
						.setAudioFileUrl(settings
								.getCmsliteUrlFor(SoundFiles.CONTINUE_PROMPTS)) }));
