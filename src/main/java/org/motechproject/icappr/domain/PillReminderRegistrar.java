...
public class PillReminderRegistrar {
	...
	@Autowired
	public PillReminderRegistrar(MRSPatientAdapter patientAdapter,
			MRSFacilityAdapter facilityAdapter,
			MessageCampaignService messageCampaignService,
			AdherenceCallEnroller adherenceCallEnroller) {
...
//		enrollInDailyMessageCampaign(registration);
	...
	private void enrollInDailyMessageCampaign(
			PillReminderRegistration registration) {
		...
		request.setReferenceTime(new Time(DateTime.now().getHourOfDay(),
	...
	private void createGenericPatient(PillReminderRegistration registration) {
		...
		attributes.add(new MRSAttributeDto(MrsConstants.PERSON_LANGUAGE_ATTR,
				registration.getPreferredLanguage()));
		attributes.add(new MRSAttributeDto(
				MrsConstants.PERSON_PHONE_NUMBER_ATTR, registration
						.getPhoneNumber()));
		attributes.add(new MRSAttributeDto(MrsConstants.PERSON_PIN_ATTR,
				registration.getPin()));
		attributes.add(new MRSAttributeDto(
				MrsConstants.PERSON_NEXT_CAMPAIGN_ATTR, registration
						.nextCampaign()));

		person.setAttributes(attributes);

		MRSPatient patient = new MRSPatientDto(null, mrsFacilityDto, person,
	...
	public PillReminderRegistration getRegistrationForPatient(String patientId) {
		...
		registration.setNextCampaign(MRSPersonUtil.getAttrValue(
				MrsConstants.PERSON_NEXT_CAMPAIGN_ATTR, attrs));
		registration.setPhoneNumber(MRSPersonUtil.getAttrValue(
				MrsConstants.PERSON_PHONE_NUMBER_ATTR, attrs));
		registration.setPin(MRSPersonUtil.getAttrValue(
				MrsConstants.PERSON_PIN_ATTR, attrs));
